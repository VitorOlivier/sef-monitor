    ...
    try {
        ...
        if (page.url().includes(config.urlLogin)) {
            ...
            await Promise.all([
                ...
                page.click(config.loginBtn),
            ]);  
            await page.goto(config.urlAgendamento,{waitUntil: 'networkidle2'});        
        }
        const servicos = await page.$$eval(config.comboBoxServicos+' option', opts => {
            return (opts.filter(opt => opt.getAttribute('value') !== '')).map((opt) => {
        ...
        for (var servico of servicos) {
            if (servico[0] !== '') {
                await Promise.all([
                    page.waitForNavigation(),
                    page.select(config.comboBoxServicos, servico[0]),
                ]);
                ...
                for (var lugar of lugares) {
                    await Promise.all([
                        page.waitForNavigation(),
                        page.select(config.comboBoxLugares, lugar[0]),
                    ]);
                    ...
                    if (!await page.$(config.msgNotFound)){
                        for(var qtdMes = 0;qtdMes < 6;qtdMes++){
                            const ths = await page.$$eval(config.ths, ths => ths.map(th => [th.getAttribute('data-date'),th.getAttribute('class')]));
